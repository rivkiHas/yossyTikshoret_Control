...
export const isBrunchStepComplete = createSelector(
  ...
  (brunches) => {
    return brunches.every((brunch) => {
      ...
      const weekdayFilled =
        weekday?.morning?.open && weekday?.morning?.close && weekday?.evening?.open && weekday?.evening?.close
      const hoursFilled = hoursOpen.every(
...
export const isAllStepsComplete = createSelector(
  ...
  (pertipComplete, contactComplete, brunchComplete) => pertipComplete && contactComplete && brunchComplete
)
