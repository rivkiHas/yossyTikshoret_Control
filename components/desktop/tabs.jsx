...
import { isBrunchStepComplete, isContactStepComplete, isPertipStepComplete } from '@/store/selectors'
...
export function Tabs({ className, ...props }) {
  ...
  const generateSteps = () => {
    const steps = [
      { id: 'step1', type: 'main', title: 'שלב ראשון', subtitle: 'פרטים על העסק' },
      {
        ...
        type: 'main',
        title: 'שלב שני',
        subtitle: 'כתובת ושעות נוספות',
        disabled: typeMarketer === 'store' && brunches.length > 1,
      },
    ]
  ...
  const getTabClass = (step, selected) => {
    ...
    const disabledClass =
      step.id === 'step2' && typeMarketer === 'store' && brunches.length > 1
  ...
  return (
    <TabGroup selectedIndex={activeStep} onChange={handleTabChange} vertical>
                    ...
                    {({ selected }) => (
                      <button className={getTabClass(step, selected)}>
                        ...
                        <div className="flex cursor-pointer flex-col justify-center">
                          {step.type === 'branch' ? (
                            <span className="text-[16px] leading-snug font-normal">{step.title}</span>
                          ) : (
                            <>
                              <div className="text-[22px] leading-snug font-bold">{step.title}</div>
                              <div className="text-[16px] leading-snug font-normal">{step.subtitle}</div>
                            </>
                          )}
                        </div>
                      </button>
