    ...
    return (
        <TabGroup selectedIndex={selectedIndex} onChange={setSelectedIndex} vertical>
            <div className={cn('flex min-h-[888px] space-x-[36px]', className)} {...props}>
                <div className="w-[368px] flex-none rounded-[40px] bg-white p-10 ps-20">
                    <Header />
                    <div className="relative border-s-2 border-amber-300">
                        <TabList className="flex translate-x-10 flex-col space-y-10">
                            <Tab as={Fragment}>
                                {({ hover, selected }) => (
                                    <button className={cn(
                                        'text-start ps-6 pe-6 py-4 w-full',
                            ...
                            {brunches.slice(1).map((brunch) => (
                                <Tab as="div" key={brunch.id}>
                                    {({ selected }) => (
                                        <button
                                            className={cn(
                                                'flex items-center justify-between w-full px-4 py-3 text-right rounded-[16px] gap-2',
                                                selected && 'bg-[#FEF2CC]'
                                            )}
                                        >
